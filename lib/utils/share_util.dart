...
class ShareUtil {
  ...
  static Future<Uint8List?> capture(GlobalKey key, {Rect? cropArea}) async {
    // Added cropArea parameter
    try {
      final boundary =
      ...
      if (boundary.debugNeedsPaint) {
        ...
        return capture(
          key,
          cropArea: cropArea,
        ); // Pass cropArea in recursive call
      }
      ...
      if (cropArea != null) {
        ...
        final canvas = ui.Canvas(
          recorder,
          Rect.fromLTWH(0, 0, scaledCropWidth, scaledCropHeight),
        );
  ...
  static Future<XFile?> saveToTempFile(Uint8List bytes, String fileName) async {
    try {
      ...
      final path =
          '${dir.path}/$fileName.png'; // Ensure .png extension for consistency
  ...
  static Future<Uint8List?> cropImageBytes(
    ...
    try {
      final ui.Image originalUiImage = await decodeImageFromList(
        originalImageBytes,
      ); // Changed to use public static method

      final recorder = ui.PictureRecorder();
      // Canvas for the cropped image, dimensions are from cropRectPx
      final canvas = ui.Canvas(
        recorder,
        Rect.fromLTWH(0, 0, cropRectPx.width, cropRectPx.height),
      );
